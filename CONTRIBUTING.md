...
The GitHub Release also gets `checksums.txt.sig` and `checksums.txt.intoto.jsonl`. Cosign v3 `sign-blob` needs `--new-bundle-format=false --use-signing-config=false` to write those detached files.
