...
Lookup is the same as every official SDK: `dataDir`, else `CLUSDR_DATA_DIR`, else `~/.clusdr`. Callers do not pass PEM bytes. Missing files throw `ClusdrError` ([Errors](../reference/errors.md#tls)).
